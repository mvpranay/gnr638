"""
Layer-Wise Feature Probing with ResNet50
...
- early_layer1  -> output of `model.layer1` (low-level textures/edges)
- middle_layer3 -> output of `model.layer3` (mid-level parts/structures)
- final_layer4  -> output of `model.layer4` (high-level semantic features)
...
# ==============================
# DATA_ROOT = "/kaggle/input/datasets/avinashc5/train-data/train_data"  # Path to dataset root in ImageFolder format
DATA_ROOT = "../train_data"
OUTPUT_DIR = "./layer_wise_feature_probing_outputs"
...
def create_feature_extractor(device: torch.device):
	...
	selected_layers = {
		"early_layer1": model.layer1,
		"middle_layer3": model.layer3,
		"final_layer4": model.layer4,
	}
...
def extract_features(
	...
	for images, y in tqdm(data_loader, desc="Extracting features", leave=False):
		...
		_ = model(images)  # forward hooks capture layer outputs

		for name in layer_names:
			fmap = activations[name]  # [B, C, H, W]
			vec = F.adaptive_avg_pool2d(fmap, output_size=1).flatten(1)  # [B, C]
...
def plot_accuracy_vs_depth(val_acc: Dict[str, float]) -> None:
	layer_order = ["early_layer1", "middle_layer3", "final_layer4"]
	...
	plt.title("Validation Accuracy vs Depth (ResNet50 Linear Probes)")
	plt.grid(alpha=0.3)
	plt.tight_layout()
...
# Fixed subset: same samples across all layers (requirement)
...
print("\nDone.")
print("Saved plots:")
print("  - val_accuracy_vs_depth.png")
print("  - feature_norm_stats.png")
print("  - pca_2d_across_layers.png")
print("  - val_accuracy.json")
print("  - feature_norm_stats.json")
