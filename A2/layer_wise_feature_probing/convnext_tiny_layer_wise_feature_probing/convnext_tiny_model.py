"""
Layer-Wise Feature Probing with ConvNeXt-Tiny
...
- early_features_1  -> output of `model.features[1]` (early local structure)
- middle_features_5 -> output of `model.features[5]` (mid-level abstraction)
- final_features_7  -> output of `model.features[7]` (deep semantic abstraction)
...
def create_feature_extractor(device: torch.device):
    ...
    selected_layers = {
        "early_features_1": model.features[1],
        "middle_features_5": model.features[5],
        "final_features_7": model.features[7],
    }
...
def plot_accuracy_vs_depth(val_acc: Dict[str, float]) -> None:
    layer_order = ["early_features_1", "middle_features_5", "final_features_7"]
    ...
    plt.title("Validation Accuracy vs Depth (ConvNeXt-Tiny Linear Probes)")
    plt.grid(alpha=0.3)
    plt.tight_layout()
