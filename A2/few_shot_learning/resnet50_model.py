...
class FewShotLearningAnalysis:
    ...
    def get_data_loaders(self, data_percentage=100, batch_size=32, val_split=0.2):
        ...
        # Load image folder dataset from provided path
        ...
        # Split full dataset into train/validation
        indices = np.random.permutation(len(full_dataset))
        val_size = int(len(full_dataset) * val_split)
        val_indices = indices[:val_size]
        train_indices_full = indices[val_size:]

        # Create train subset based on percentage (few-shot regime)
    ...
    def train_resnet50(self, data_percentage=100, epochs=20, lr=0.001):
        """Train ResNet50 with specified data percentage."""
    ...
    def analyze_few_shot_learning(self):
        ...
        for pct in tqdm(percentages, desc="Few-shot regimes"):
            ...
            model, results = self.train_resnet50(data_percentage=pct)
...
analyzer = FewShotLearningAnalysis(seed=42)
