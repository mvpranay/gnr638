...
# CONFIG
# ----------------------------
CHECKPOINT_PATH = "convnext_tiny_aid.pth"
...
class FeatureExtractor(nn.Module):
    def __init__(self, model):
        ...
        self.model = model
        self._features = None
        # ConvNeXt's classifier is Sequential with LayerNorm and Linear
        # Hook the pre-call to classifier to capture the 768-d input
        self._hook_handle = self.model.classifier[0].register_forward_pre_hook(
            self._save_features
        )

    def _save_features(self, module, input):
        # input is a tuple; input[0] is the flattened feature vector (N, 768)
