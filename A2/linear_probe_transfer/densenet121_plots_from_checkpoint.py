...
# CONFIG
# ----------------------------
CHECKPOINT_PATH = "densenet121_aid.pth"
...
class FeatureExtractor(nn.Module):
    def __init__(self, model):
        ...
        self.model = model
        self._features = None
        # DenseNet's avg pool is inline in forward(); hook the classifier's
        # pre-hook instead to capture the flattened 1024-d input vector.
        self._hook_handle = self.model.classifier.register_forward_pre_hook(
            self._save_features
        )

    def _save_features(self, module, input):
        # input is a tuple; input[0] is the flattened feature vector (N, 1024)
