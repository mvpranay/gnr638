...
To evaluate representation quality beyond a single clean accuracy number, we follow a multi-scenario protocol implemented in our codebase: (i) linear probe transfer with a frozen backbone and trainable final classifier, (ii) few-shot analysis by training on $100\%$, $20\%$, and $5\%$ of training data, (iii) layer-wise probing using linear classifiers on early/middle/final intermediate features, and (iv) corruption robustness testing under Gaussian noise, motion blur, and brightness shift. Together, these analyses characterize transferability, data efficiency, feature hierarchy, and robustness sensitivity of pre-trained representations.
...
\begin{table}[H]
	...
	\begin{tabular}{lcc}
		...
		EfficientNet-B0 & Not exported in text log & Not exported in text log \\
		ConvNeXt-Tiny & Not exported in text log & Not exported in text log \\
		\bottomrule
	\end{tabular}
\end{table}

From the available numeric log (ResNet50), linear-probe transfer is very strong, with peak and final validation accuracy both reaching $98.28\%$. For EfficientNet-B0 and ConvNeXt-Tiny, checkpoint diagnostics (confusion matrix, PCA, t-SNE, and training-validation curves) are available in the folder and indicate strong class separation overall; exact epoch-wise numeric arrays were not exported as plain-text logs in the current artifacts.
...
Across all three models, training-validation curves show rapid early improvement followed by stabilization, consistent with strong transfer from ImageNet-pretrained features. The ResNet50 log confirms a high final train accuracy (99.75\%) and high final validation accuracy (98.28\%), indicating limited generalization gap in this setting.
...
\begin{table}[H]
	...
	\begin{tabular}{lcccc}
		\toprule
		Model & Few-Shot Score & Probe Score & Transfer Score & Robustness Score \\
		\midrule
		ResNet50 & 50.29 (Acc@5\%) & 94.92 (best probe acc) & 98.28 (val acc) & 59.64 (mean corrupt acc) \\
		EfficientNet-B0 & 70.03 (Acc@5\%) & 92.92 (best probe acc) & strong (plots only) & 25.64 (mean corrupt acc) \\
		ConvNeXt-Tiny & 61.87 (Acc@5\%) & 95.35 (best probe acc) & strong (plots only) & 68.36 (mean corrupt acc) \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{itemize}
	...
	\item \textbf{Cross-task insight:} Backbone selection should be objective-driven (data efficiency vs robustness vs transfer), rather than based on a single metric.
\end{itemize}

Main limitations of the current study are:
\begin{itemize}
	\item Some linear-probe numeric logs were not exported as text for all models, so part of the transfer comparison relies on plot-based qualitative evidence.
	\item Fine-tuning strategy analysis (linear probe vs partial unfreezing vs full fine-tuning) is intentionally deferred and therefore excluded from cross-task conclusions.
\end{itemize}

Future work directions include:
\begin{itemize}
	\item Complete and integrate fine-tuning strategy experiments with consistent logging of convergence and robustness changes.
	\item Add stronger robustness-oriented training interventions (e.g., corruption augmentation, adversarial-style noise training, or domain adaptation for aerial imagery).
\end{itemize}
